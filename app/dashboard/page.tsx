...
import { useEffect, useState, useMemo, useCallback, ReactNode, Fragment, ChangeEvent, useRef } from 'react'
import { auth, db } from '@/lib/firebase'
import { onAuthStateChanged, signOut, User } from 'firebase/auth'
import { doc, setDoc, getDoc, collection, query, onSnapshot, updateDoc, Timestamp } from 'firebase/firestore'
import { useRouter } from 'next/navigation'
// ✅ MODIFICATION: Added 'isTomorrow' and 'isAfter' for improved date/time logic
import {
...
import {
    ...
    // ✅ MODIFICATION: Added QR Code icons
    QrCode as QrCodeIcon, Download,
} from 'lucide-react'
import { QRCodeCanvas } from 'qrcode.react'; // ✅ ADDED: For QR Code generation
...
// ✅ FIX: Updated Booking interface to match the actual data structure in Firestore.
// This ensures that requester details are correctly read and used for sending emails.
interface Booking {
    id: string;
...
const VISIBLE_BOOKINGS_LIMIT = 2; // Limit bookings shown per day in calendar
...
const SmartAssistant = ({
    ...
    const assistantState = useMemo(() => {
        ...
        // ✅ ENHANCEMENT: Differentiate between all of today's meetings and upcoming ones.
        const now = new Date();
        const allTodaysBookings = bookings
            .filter(b => b.status === 'accepted' && isToday(new Date(b.date)))
            .sort((a, b) => a.time.localeCompare(b.time));

        const upcomingTodaysBookings = allTodaysBookings.filter(booking => {
            const meetingDateTime = parse(booking.time, 'HH:mm', new Date(booking.date));
            return isValid(meetingDateTime) && isAfter(meetingDateTime, now);
        });


        // If no primary suggestions and no meetings at all today, add a friendly message.
    ...
    const renderTimelineItemAction = (booking: Booking) => (
        <div className="flex items-center gap-1">
            ...
            <a
                href={`mailto:${booking.requesterEmail}?subject=Reminder: Meeting at ${format(parse(booking.time, 'HH:mm', new Date()), 'h:mm a')}&body=Hi ${booking.requesterName},%0D%0A%0D%0AJust a friendly reminder about our upcoming meeting today at ${format(parse(booking.time, 'HH:mm', new Date()), 'h:mm a')}.%0D%0A%0D%0ASubject: ${booking.subject}%0D%0A%0D%0ALooking forward to it!%0D%0A%0D%0ABest,%0D%0A${profileData.name}`}
    ...
    return (
        <div className={clsx(
            "bg-gradient-to-br from-blue-50 via-white to-indigo-50 border border-slate-200 p-4 sm:p-6 rounded-2xl shadow-sm transition-all duration-500 ease-in-out",
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {/* ✅ ENHANCEMENT: Check against all of today's meetings to decide the main view */}
                    {assistantState.allTodaysBookings.length > 0 ? (
                        <>
                            {/* ✅ ENHANCEMENT: Dynamic message based on whether meetings are upcoming or past */}
                            <p className="text-sm text-slate-600 mt-1.5">
                                {assistantState.upcomingTodaysBookings.length > 0 ? (
                                    <>
                                        You have <span className="font-bold">{assistantState.upcomingTodaysBookings.length}</span> upcoming meeting{assistantState.upcomingTodaysBookings.length > 1 ? 's' : ''} on your schedule.
                                    </>
                                ) : (
                                    <>
                                        You had <span className="font-bold">{assistantState.allTodaysBookings.length}</span> meeting{assistantState.allTodaysBookings.length > 1 ? 's' : ''} today.
                                    </>
                                )}
                            </p>

                            {/* ✅ ENHANCEMENT: Only render the timeline for UPCOMING meetings */}
                            {assistantState.upcomingTodaysBookings.length > 0 && (
                                                        ...
                                                        <div className="min-w-0 flex-1 bg-white/70 backdrop-blur-sm border border-slate-200/80 rounded-lg p-3 pt-2.5">
                                                            <div className="flex justify-between items-center">
                                                                <div className="min-w-0">
                                                                    ...
                                                                    <p className="font-semibold text-slate-800 text-sm mt-0.5 truncate">{booking.subject}</p>
                                                                    <p className="text-xs text-slate-500">with {booking.requesterName}</p>
                                                                </div>
                        ...
                        assistantState.primarySuggestion && (
                         <div className="mt-2 bg-white/60 p-4 rounded-lg border border-slate-200/80 flex flex-col sm:flex-row items-start gap-4">
                              <div className="text-blue-600 bg-blue-100 p-2 rounded-full mt-1 shrink-0">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200/80 flex flex-wrap gap-2">
                        {assistantState.allTodaysBookings.length === 0 && assistantState.primarySuggestion?.type === 'info' && (
...
const CollapsibleSection = ({ id, title, icon, summary, children, defaultOpen = false }: { id?: string; title: string, icon: ReactNode, summary?: string, children: ReactNode, defaultOpen?: boolean }) => (
    <Disclosure as="div" id={id} className="bg-white border border-slate-200 rounded-2xl shadow-sm" defaultOpen={defaultOpen}>
        {({ open }) => (
            <>
                <Disclosure.Button className="w-full flex justify-between items-center gap-4 p-4 sm:p-6 text-left transition-colors hover:bg-slate-50/50 rounded-t-2xl">
                    <div className="flex items-center gap-4 min-w-0">
                        <div className="text-slate-500 shrink-0">{icon}</div>
                        <div className="min-w-0">
                            <h2 className="text-lg font-bold text-slate-800 truncate">{title}</h2>
                            {summary && <p className="text-sm text-slate-500 truncate">{summary}</p>}
                        </div>
                    </div>
        ...
        >
            <div className="border-t border-slate-200 p-4 space-y-4 overflow-hidden">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <p><strong className="font-semibold text-slate-600 block">Email:</strong> <a href={`mailto:${booking.requesterEmail}`} className="text-blue-600 hover:underline">{booking.requesterEmail}</a></p>
                    {booking.requesterPhone && <p><strong className="font-semibold text-slate-600 block">Phone:</strong> {booking.requesterPhone}</p>}
...
const CancellationModal = ({ isOpen, onClose, onConfirm, booking, note, setNote, isCancelling }: { isOpen: boolean; onClose: () => void; onConfirm: () => void; booking: Booking | null; note: string; setNote: (note: string) => void; isCancelling: boolean }) => {
 ...
 return <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog"><div className="bg-white rounded-xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}><div className="p-4 sm:p-6"><h3 className="text-lg font-bold text-slate-900">Cancel Booking</h3><p className="mt-2 text-sm text-slate-500">Are you sure you want to cancel with <span className="font-semibold">{booking.requesterName}</span>?</p><div className="mt-4"><label htmlFor="cancellationNote" className="block text-sm font-medium text-slate-700">Note (Optional)</label><textarea id="cancellationNote" rows={4} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Let them know why..." className="mt-1 block w-full border-slate-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" /></div></div><div className="bg-slate-50 px-4 py-3 sm:px-6 sm:py-4 flex flex-col-reverse sm:flex-row-reverse gap-3 rounded-b-xl"><button type="button" onClick={onConfirm} disabled={isCancelling} className="w-full sm:w-auto justify-center rounded-md border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 disabled:bg-red-300">{isCancelling ? 'Cancelling...' : 'Yes, Cancel'}</button><button type="button" onClick={onClose} disabled={isCancelling} className="w-full sm:w-auto justify-center rounded-md border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50">Keep Booking</button></div></div></div>
}
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="p-4 sm:p-6">
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                        Select a new time for your meeting with <span className="font-semibold">{booking.requesterName}</span> on <span className="font-semibold">{format(new Date(booking.date), 'EEEE, MMM d')}</span>.
                    </p>
                    <div className="mt-4">
                ...
                <div className="bg-slate-50 px-4 py-3 sm:px-6 sm:py-4 flex flex-col-reverse sm:flex-row-reverse gap-3 rounded-b-xl">
                    <button
                        type="button"
                        onClick={onConfirm}
...
// --- ✅ RESTYLED COMPONENT: Month View Calendar ---
        ...
        <div className="p-4 sm:p-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center">
                    <button onClick={prevMonth} className="p-1.5 text-slate-500 hover:text-slate-800 transition-colors" aria-label="Previous month">
                        <ChevronLeft size={20} />
                    </button>
                    <button onClick={nextMonth} className="p-1.5 text-slate-500 hover:text-slate-800 transition-colors" aria-label="Next month">
            ...
            <div className="overflow-x-auto">
                 <div className="min-w-[560px]">
                    ...
                    <div className="mt-4 grid grid-cols-7 text-center text-xs font-semibold text-slate-500 border-b border-slate-200">
                    ...
                    <div className="grid grid-cols-7">
                        ...
                        {Array.from({ length: startingDayIndex }).map((_, i) => (
                            <div key={`empty-${i}`} className="border-r border-b border-slate-200 bg-slate-50" />
                        ))}

                        {/* Day cells */}
                        {daysInMonth.map((day, dayIdx) => {
                            const dateKey = format(day, 'yyyy-MM-dd');
                            const dayBookings = acceptedBookingsByDate[dateKey] || [];
                            ...
                            return (
                                <div
                                    key={day.toString()}
                                    className={clsx(
                                        "relative h-32 md:h-36 p-2 border-b border-r border-slate-200 flex flex-col",
                                        (startingDayIndex + dayIdx + 1) % 7 === 0 && 'border-r-0',
                                ...
                                >
                                    <time
                                        dateTime={dateKey}
                                        className={clsx(
                                            "text-sm font-semibold flex items-center justify-center h-7 w-7 rounded-full mb-1",
                                            isCurrentToday && "bg-blue-600 text-white",
                                            !isCurrentToday && isCurrentMonth && "text-slate-800",
                                    ...
                                    {dayBookings.length > 0 && (
                                        <div className="flex-grow overflow-y-auto no-scrollbar">
                                            <ul className="space-y-1">
                                                {dayBookings.slice(0, VISIBLE_BOOKINGS_LIMIT).map(booking => (
                                                    <li key={booking.id}>
                                                        <div
                                                            className="bg-green-100 text-green-800 rounded px-2 py-1 text-xs font-medium truncate"
                                                ...
                                                {dayBookings.length > VISIBLE_BOOKINGS_LIMIT && (
                                                    <li className="text-xs text-slate-500 font-medium pt-1">
                                                        + {dayBookings.length - VISIBLE_BOOKINGS_LIMIT} more
                                                    </li>
                                                )}
                                            </ul>
                                        </div>
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <div className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                                <div className="p-4 sm:p-6">
                                    ...
                                    <div className="mt-6 space-y-4 max-h-[60vh] overflow-y-auto pr-3 -mr-3">
                                        {(availability[dayKey] || []).map((slot, index) => {
                                            ...
                                            return (
                                                <div key={index} className={clsx("relative grid grid-cols-1 sm:grid-cols-4 items-center gap-y-4 sm:gap-x-3 p-3 rounded-lg", isBooked ? "bg-red-50" : "bg-slate-100")}>
                                ...
                                <div className="bg-slate-50 px-4 py-3 sm:px-6 sm:py-4 flex justify-end">
...
export default function DashboardPage() {
    ...
    const qrCodeRef = useRef<HTMLDivElement>(null); // ✅ ADDED: Ref for QR Code download

    // ✅ FIX: Updated greeting logic for the main dashboard heading.
    const smartGreeting = useMemo(() => {
        const name = profileData.name?.split(' ')[0] || '...';
        const currentHour = getHours(new Date());
        if (currentHour >= 5 && currentHour < 12) { // 5am - 11:59am
            return `Good morning, ${name}!`;
        }
        if (currentHour >= 12 && currentHour < 17) { // 12pm - 4:59pm
            return `Good afternoon, ${name}.`;
        }
        if (currentHour >= 17 && currentHour < 21) { // 5pm - 8:59pm
 ...
 const publicLink = useMemo(() => {
  ...
  return `${baseUrl.replace(/\/$/, '')}/u/${username}`;
    ...
    // ✅ ADDED: Memoize vCard data for QR Code
    const vCardData = useMemo(() => {
        if (!user?.email || !profileData.name) return '';
        const parts = [
            'BEGIN:VCARD',
            'VERSION:3.0',
            `FN:${profileData.name || ''}`,
        ...
        if (profileData.whatsapp) {
            // Attempt to extract a number, assuming it might be a wa.me link or just numbers
            const phone = profileData.whatsapp.replace(/[^0-9]/g, '');
            if(phone) {
                parts.push(`TEL;TYPE=WORK,VOICE:${phone}`);
            }
        }
        if (publicLink) {
            parts.push(`URL:${publicLink}`);
        }
        parts.push('END:VCARD');
        return parts.join('\n');
    }, [profileData, user, publicLink]);

    // ✅ ADDED: Handler to download the QR code canvas as a PNG
    const handleDownloadQRCode = () => {
        ...
        if (canvas) {
            ...
            const fileName = (profileData.name || 'contact').replace(/[^a-z0-9]/gi, '_').toLowerCase();
    ...
    // ✅ FIX: This is the primary logic change to fix the "Next Meeting" card.
    const dashboardStats = useMemo(() => {
        ...
        // Find the *first upcoming* meeting, not just the first of the day.
    ...
    const handleUpdateStatus = async (bookingId: string, status: BookingStatus) => {
        ...
        try {
            ...
            if (status === 'accepted') {
                ...
                if (acceptedBooking) {
                    // ✅ FIX: Use correct fields (requesterEmail, requesterName) for the API call.
                    fetch('/api/send-confirmation', {
                        method: 'POST', headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            ownerEmail: user.email, ownerName: profileData.name || username,
                            requesterEmail: acceptedBooking.requesterEmail, requesterName: acceptedBooking.requesterName,
    ...
    const handleConfirmCancel = async () => {
        ...
        try {
            ...
            await fetch('/api/send-cancellation', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ownerName: profileData.name || username, requesterEmail: bookingToCancel.requesterEmail,
                    requesterName: bookingToCancel.requesterName, date: bookingToCancel.date,
        ...
        <>
            ...
            <div className="bg-slate-50 min-h-screen">
                ...
                <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
                    <div className="mb-8">
                        {/* ✅ FIX: Greeting is now dynamic based on time of day */}
                        <h2 className="text-2xl sm:text-3xl font-bold text-slate-800">{smartGreeting}</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
                        ...
                        <StatCard title="Next Meeting" value={dashboardStats.nextMeeting ? `${format(parse(dashboardStats.nextMeeting.time, 'HH:mm', new Date()), 'h:mm a')}` : "None"} note={dashboardStats.nextMeeting ? `with ${dashboardStats.nextMeeting.requesterName}` : "Enjoy your day!"} icon={<Clock />} />
                    ...
                    {username && publicLink && (
                        <div className="p-4 sm:p-6 bg-white border border-slate-200 rounded-2xl shadow-sm mb-8">
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                                    {/* ✅ FIX: LinkedIn button moved first */}
                                    <a
                                    ...
                                    {/* ✅ FIX: WhatsApp button now conditional based on profile data */}
                    ...
                    <div className="space-y-6">
                        ...
                        <CollapsibleSection id="profile-section" title="Profile Editor" icon={<UserIcon size={22} />} summary="Manage your public information">
                            <div className="border-t border-slate-200 p-4 sm:p-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-2 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                            {(imagePreview || profileData.photoUrl) ? (
                                                ...
                                                <div className="h-24 w-24 flex items-center justify-center rounded-full bg-slate-100 text-slate-400 border-2 border-white shadow-sm shrink-0">
                                                    <UserIcon size={48} />
                                                </div>
                                            )}
                                     ...
                                     <div className="pt-6 border-t border-slate-200 space-y-4">
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                              ...
                                              <label className="block">
                                                  ...
                                                  <input type="text" value={profileData.whatsapp || ''} onChange={(e) => setProfileData({...profileData, whatsapp: e.target.value})} className="mt-1 block w-full border-slate-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="e.g. +1234567890"/>
                                              </label>
                                              <label className="block">
                                ...
                                <div className="mt-6 flex items-center justify-end gap-4">
                                    {showProfileSaveSuccess && <p className="text-sm font-medium text-green-600">✓ Saved!</p>}
                        ...
                        {/* ✅ NEW SECTION START: Contact QR Code */}
                        <CollapsibleSection
                            id="qr-code-section"
                            title="Contact QR Code"
                            icon={<QrCodeIcon size={22} />}
                            summary="Download a QR code to share your contact details"
                        >
                            <div className="border-t border-slate-200 p-4 sm:p-6 flex flex-col items-center gap-6">
                                <div ref={qrCodeRef} className="p-4 bg-white border border-slate-200 rounded-lg shadow-inner">
                                    <QRCodeCanvas
                                        value={vCardData}
                                        size={256}
                                        bgColor={"#ffffff"}
                                ...
                                <button
                                    ...
                                    disabled={!vCardData}
                                    className="inline-flex items-center justify-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed transition-colors shadow-sm"
                                >
                                    <Download size={16} />
                                    Download QR Code
                                </button>
                                {!profileData.name && (
                                    <p className="text-xs text-center text-red-600 mt-2">Please add your name in the Profile Editor to generate a valid QR code.</p>
                                )}
                                </div>
                            </div>
                        </CollapsibleSection>
                                ...
                                <div className="border-t border-slate-200 p-4 sm:p-6">
                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                                        ...
                                        <button onClick={applyLastWeeksSchedule} className="flex-shrink-0 flex items-center justify-center gap-2 bg-white text-blue-700 border border-slate-300 hover:border-blue-300 hover:bg-slate-50 px-3 py-2 text-sm font-semibold rounded-md transition-colors shadow-sm">
                                            <Copy size={16} /> Copy Last Week's Schedule
                                        </button>
                                    </div>
              ...
              <div className="min-w-[560px]">
                ...
                <div className="grid grid-cols-7">
                    ...
                    {days.map((day, dayIdx) => {
                        ...
                        return (
                            <div key={dayKey} className={clsx("h-28 sm:h-36 p-2 border-b border-r border-slate-100 flex flex-col relative transition-colors group", (startingDayIndex + dayIdx + 1) % 7 === 0 && 'border-r-0', isPastDay ? 'bg-slate-50' : 'cursor-pointer bg-white hover:bg-blue-50/50')} onClick={() => !isPastDay && openModal(day)}>
                                <time dateTime={dayKey} className={clsx("text-xs font-semibold self-start", isToday(day) && "flex items-center justify-center h-6 w-6 rounded-full bg-blue-600 text-white", isPastDay ? "text-slate-400" : "text-slate-700")}>
                                    {format(day, 'd')}
                                </time>
                                <div className="mt-1 flex-grow overflow-y-auto no-scrollbar">
                                    <ul className="space-y-1">
                                        {dayAvailability.map((slot, i) => {
                                            const start = slot.start ? format(parse(slot.start, 'HH:mm', new Date()), 'h:mma').replace(':00','').toLowerCase() : '';
                                            const end = slot.end ? format(parse(slot.end, 'HH:mm', new Date()), 'h:mma').replace(':00','').toLowerCase() : '';
                                            return (
                                ...
                                {!isPastDay && (
                                    <div className="absolute bottom-2 left-2 right-2 text-center text-xs font-semibold text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
